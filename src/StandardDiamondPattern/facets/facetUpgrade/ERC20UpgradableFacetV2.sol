...
contract ERC20UpgradableFacetV2 is StandardImplementationInitializerFacet, IERC20Upgradeable, IERC20MetadataUpgradeable {

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() StandardImplementationInitializerFacet("erc20"){
        _disableInitializers();
    }

    function patchVersion() external pure returns (uint256 version) {
        version = 2;
    }
    ...
    function initialize(string memory name_, string memory symbol_) initializer public {
        LibERC20.ERC20UpgradableStorage storage s = LibERC20.diamondStorage();
        s._name = name_;
        s._symbol = symbol_;
        _mint(msg.sender, 10_000_000);
    }

    function buyERC20Tokens() payable external {
        _mint(msg.sender, msg.value * 1_000);
    }
